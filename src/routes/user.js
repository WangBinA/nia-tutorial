...
module.exports = function (done) {
  ...
  $.router.post('/api/user/request_reset_password', $.checkLogin, async function (req, res, next) {
  ...
  $.router.post('/api/user/reset_password', $.checkLogin, async function (req, res, next) {
  ...
  $.router.post('/api/user/unbindoa', $.checkLogin, async function (req, res, next) {
    
    var update = {_id: req.session.user._id};
    var changed = {};
    if (req.body.unbindGithub) {
      console.log('unbinding github user:', req.session.user.githubUsername);
      update.githubUsername = null;
      changed.unbindGithub = true;
    }
    var updateResult = await $.method('user.update').call(update);
    console.log('updateResult', updateResult);
    if (!updateResult.ok) {
      return next(new Error('Unbinding failed'));
    }
    const user = await $.method('user.get').call({_id: req.session.user._id});
    req.session.user = user;
    res.apiSuccess(changed);
  });
